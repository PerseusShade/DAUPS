...
async function mergeAndBookmark(lang, chapters, locale) {
    ...
    const sanitizedCoverTop = locale.coverTop.replace(/[^a-z0-9_\-]/gi, '_');
    const outputName = `DAUPS_${sanitizedCoverTop}_${lang}.pdf`;
    const outputPath = path.resolve(`docs/site/pdf/${outputName}`);
    execSync(`pdftk "${temp}" update_info_utf8 "${bm}" output "${outputPath}"`);
    ...
    await fs.writeFile(outputPath, await finalPdf.save());
